...
When we come to the micro-benchmark, we found that the preliminary Nussinov DP method is significantly slowing down in the cases that the number of nucleotides in the RNA sequence is more than 3000. That's intrinsically caused by its $O(log(n^3))$ time complexity characteristic. While F-G method \cite{gusfield} has enhanced the performance by a factor of $log(n)$, in some cases the speed is still not satisfactory for long RNA sequences. With the introduction of CUDA onto two-vector method invented by \cite{balaji}, we almost achieve a linearly increasing time complexity for different size of RNA sequences, which is quite impressive.

In the future if \textit{Vienna} or \textit{m-fold} etc. application package employs F-G method or even the parallelized CUDA method, we could see a great speed improvement in RNA design or visualization application.
