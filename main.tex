...
\par In this paper, we performed both application and micro benchmark for current packages and some popular
...
for RNA strands of greater than 1000 nucleobases, although more testing is necessary to establish this conclusively.

\par When we come to the micro-benchmark, we found that the \textit{Nussinov} DP method becomes
appreciably slower, particularly to a human, in the cases that the number of nucleotides in the RNA sequence is
...
to linearly increasing time complexity for different size of RNA sequences. Thus when dealing with super long RNA
...
through the Nussinov method. Furthermore RNA packages like \textit{RNAfold Vienna} and \textit{m-fold} must also
take into account complex thermodynamic rules in order to find both optimal and suboptimal RNA structures which
would slow time down even more in an application setting. As a result implementation of an improved folding algorithm
like Frid-Gusfield or the parallel Four Russians introduced by Venkatachalam is imperative going forward in
the future.
